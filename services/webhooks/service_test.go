...
func TestWebhookParsing(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		c, err := session.Engine().HTTPService().Request(request, "")
		...
		if tc.isError {
			...
			assert.Equal(t, tc.webhook.request, c.Request(), "request trace mismatch for call %s", tc.call)
			assert.Equal(t, tc.webhook.response, c.Response(), "response mismatch for call %s", tc.call)
			assert.Equal(t, tc.webhook.bodyIgnored, c.BodyIgnored(), "body-ignored mismatch for call %s", tc.call)

			test.AssertEqualJSON(t, tc.webhook.json, flows.ExtractResponseBody(c.Response()), "body mismatch for call %s", tc.call)
		}
	}
}
